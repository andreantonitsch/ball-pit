...
const createBox = (size) => {
    ...
    linkBodyMesh(boxBody, boxMesh)
}

const createShape = (size) =>{
    if(Math.random() < params.sphereRatio){
        createSphere(size)
    } else {
        createBox(size * 1.5)
    }
...
for(let i = 0; i < params.sphereCount; i++){
    createShape(Math.random() * .2 + .8)
}

params.resetBalls = () =>{
    for(const pair of updateList){
        pair.body.position.set(
            normRandom() * params.floorSize * .5,
            3,
            normRandom() * params.floorSize * .5,
        )
...
gui.add(params,'resetBalls')
params.resetBalls()

params.createShape = () =>{
    createShape(Math.random() * .3 + .7)
}
gui.add(params, 'createShape')
